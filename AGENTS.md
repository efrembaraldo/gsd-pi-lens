...
diagnostic rather than a silent mount failure. (#1381)
  ...
  widget-state.ts         Footer widget rendering (@gsd/pi-tui)
...
All pi packages in this fork are `@gsd/*`. Optional peer deps: `@gsd/pi-coding-agent` (type-only, the host SDK) and `@gsd/pi-tui` (value, `^1.17.0`, alongside optional `typebox`). There is no SDK devDependency: types resolve through the `vendor/` copies (`vendor/pi-coding-agent`, `vendor/pi-tui`) plus `scripts/setup-types.mjs`, which materializes them from a gsd-pi checkout. Re-audit host declarations before taking a future major/minor bump.
...
A 2026 audit against `@gsd/pi-coding-agent` confirmed a few places where pi-lens intentionally does *not* reuse an SDK facility:
...
**First-class seam census (pinned by `tests/config/rpc-bus-conformance.test.ts`, R009 / S07).** A single module owns BOTH ends of the request/response bus today: `clients/rpc-publish.ts` is both the **publisher** (emits `pilens:rpc:<token>:response` carrying `PilensDiagnosticsPayload` verbatim for the diagnostics channel and a thin `{paths, ts}` envelope for files-touched) and the **subscriber** (listens on `pilens:rpc:diagnostics` and `pilens:rpc:files-touched`, reads `widgetState.allDiagnostics` plus the recent-touches buffer, then re-emits per-token). The RPC surface intentionally breaks the "broadcast-only" doctrine of #482 / #502 — see "Three channels, three audiences" below. The conformance test sweeps `clients/`, `tools/`, `mcp/`, `scripts/` plus root `index.ts` for any module that subscribes to a request channel or emits on a `pilens:rpc:*` response channel, and pins the response-channel helper shape (`rpcResponseChannel(token)` concatenating `pilens:rpc:` + token + `:response`) so a future refactor that switches to a non-template construction trips the guard.
...
### `pilens:rpc:*` — the request/response bus surface (R009 / S07)

`clients/rpc-publish.ts` is a sibling module to `clients/bus-publish.ts` and `clients/diagnostics-publish.ts` (the latter owns the `PilensDiagnosticsPayload` shape the RPC layer reuses verbatim). It is the FIRST surface that breaks the "broadcast-only" doctrine of the #482 / #502 family: pi's `pi.events` is a plain EventEmitter with no native request/response semantics, so the module reserves two request channels (`pilens:rpc:diagnostics`, `pilens:rpc:files-touched`) and emits a correlated response on `pilens:rpc:<token>:response` for every well-formed request. The token in the channel name is the structural loop guard — a requester never sees another requester's reply, and the response channel is constructed by the internal `rpcResponseChannel(token)` helper, not a literal at the call site. `tests/config/rpc-bus-conformance.test.ts` pins the publisher + subscriber census (a single module owns both ends today), the response-channel helper shape (prefix + token + suffix), and the AGENTS.md citation contract. **Note for the next agent who adds another `pilens:rpc:*` channel:** the `origin: "bus"` structural loop guard on `clients/bus-publish.ts` does NOT extend to this surface — a `publishFilesTouched` publication is fire-and-forget, so a same-process consumer that re-emits it would loop forever; an `pilens:rpc:<token>:response` emission, by contrast, only answers a specific request, and the token-scoped channel name makes a re-request visible as a separate request to a future emulator. Do not import `origin: "bus"` semantics here — the token IS the loop guard, and the request envelope is the only thing that ever produces a response.
...
**Shared schema with #478 (now shipped via R009 / S07, bound, not merged):** `PilensDiagnosticsPayload` is defined once in `clients/diagnostics-publish.ts` and reused verbatim by the RPC pull response — push (`pilens:diagnostics`) and pull (`pilens:rpc:diagnostics`) are two deliveries of the same shape over the same lens-engine seam. `#449` registry dogfooding remained a parallel gating concern but did NOT block the R009 ship: the RPC layer only reads the in-memory widget state, not the cross-process instance registry, so the unblocking dependency was confined to the `PilensDiagnosticsPayload` type already exported by the diagnostics publisher. Adding a second pull channel (e.g. for `pilens:rpc:knip-findings`) means adding one new `handleXxxRequest` plus one new `BUS_RPC_REQUEST_XXX_EVENT` constant — the schema, cap, and TTL machinery are shared and live in `clients/runtime-config.ts` (`getRpcMaxDiagnosticsPerResponse`, `getRpcResponseTtlMs`).
