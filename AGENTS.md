...
All pi packages are `@gsd/*` (migrated from `@mariozechner/*` in 0.74.0). Peer dep: `@gsd/pi-coding-agent`. Runtime dep: `@gsd/pi-tui`. The v4-safe dependency baseline resolves both host packages at `0.84.2`; the peer remains broad at runtime and the devDependency pins the SDK for type/compatibility checks. Re-audit host declarations before taking a future major/minor bump.
...
**Shared schema with #478/R009 (bound, not merged):** `PilensDiagnosticsPayload` is defined once in `clients/diagnostics-publish.ts` and is reused verbatim by R009's `pilens:rpc:diagnostics` pull response — push (this event) and pull (R009) are two deliveries of the same shape over the same lens-engine seam.

### `pilens:rpc:*` — the pull surface (#478, R009, shipped M001/S07)

`clients/rpc-publish.ts` closes the pull-side gap the two push events above leave open: a same-process listener that starts up mid-session (or missed an earlier push) can ask for the CURRENT state instead of waiting for the next change. It reuses the existing `pi.events` bus rather than opening a second channel: a requester emits `pilens:rpc:diagnostics` or `pilens:rpc:files-touched` (the module's own `BUS_RPC_REQUEST_DIAGNOSTICS_EVENT`/`BUS_RPC_REQUEST_FILES_TOUCHED_EVENT` constants) carrying a correlation token, and `wireRpcBusSubscriber` (called once at extension factory time, same seam as `wireBusEmitter`) answers on a token-scoped response channel (`pilens:rpc:<token>:response`, `rpcResponseChannel`), reusing `PilensDiagnosticsPayload` verbatim for the diagnostics reply so a consumer parses ONE shape whether it arrived by push or pull. Kill switch: `isBusPublishEnabled()`, the same `PI_LENS_BUS_PUBLISH` gate the push events share — a disabled bus answers no requests, it does not silently degrade.

**Bounds, load-bearing not cosmetic:** `rpc.maxDiagnosticsPerResponse` caps entries per reply (mirrors the push side's per-file cap, applied per-response here); `rpc.responseTtlMs` (default 5000) bounds how long a request stays answerable — `requestReceivedAt`, a module-scope `Map<token, receivedAt>`, tracks each in-flight token's arrival time and a lazy sweep (triggered on the next request, not a timer) drops entries past their TTL before touching the map, so a stale token from a slow or abandoned requester is never answered into a listener that has moved on. An empty or non-string token is ignored (no response, no error) rather than crashing the subscriber on a malformed request. `resetRpcPublishSessionState()` clears both the token map and the subscriber's wiring state at `session_start` (`clients/runtime-session.ts`) — both are durable for a SESSION, not the process (see the session-state registry in `tests/support/session-state-registry.ts`).

**Conformance:** `tests/config/rpc-bus-conformance.test.ts` pins the request event constants, the response-channel template literal, and the publisher/subscriber module census — the same declare-and-explain discipline as `pilens:files:touched` above.
