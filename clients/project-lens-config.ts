...
function discoverPiLensProjectConfig(startDir: string): DiscoveryCacheEntry {
	...
	for (const dir of configSearchDirs(startDir)) {
		...
		for (const location of LEGACY_PROJECT_LOCATIONS) {
			...
			// #2426 follow-up: when this legacy-named file IS the document that
			// `info` resolves to, its deprecation records are already produced by
			// the resolution path's own `deprecationRecords([document])` call, in
			// the SAME bound as its validation records
			// (`resolveOnePiLensConfigDocument`). Adding it here too finalizes an
			// independent bound over the identical document, which can truncate to
			// a different count and defeat the warn-once latch's exact-reason
			// dedup -- two suppression notices for one file
			// (config-notice-bounds.test.ts F2).
